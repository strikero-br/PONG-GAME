 ...
 */
function moverBola() {
  // Move a bola primeiro
  bola.x += (bola.velocidade + bola.mod) * bola.dirx;
  bola.y += (bola.velocidade + bola.mod) * bola.diry;

  // Colisão com as bordas superior e inferior
  if (bola.y <= 0) {
    bola.y = 0; // Evita que a bola fique fora do canvas
    bola.diry = -bola.diry;
    reproduzirSom(somBatida);
  } else if (bola.y + bola.altura >= canvas.height) {
    bola.y = canvas.height - bola.altura; // Evita que a bola fique fora do canvas
    bola.diry = -bola.diry;
    reproduzirSom(somBatida);
  }
  ...
  if (
    bola.dirx < 0 && // Só verifica se a bola está se movendo para a esquerda
    ...
    bola.x = jogadorEsquerda.x + jogadorEsquerda.largura; // Ajusta a posição da bola para evitar múltiplas colisões
    bola.dirx = 1;
    bola.mod += 0.2;
    reproduzirSom(somBatida);
  ...
  if (
    bola.dirx > 0 && // Só verifica se a bola está se movendo para a direita
    bola.y + bola.altura >= jogadorDireita.y &&
    bola.y <= jogadorDireita.y + jogadorDireita.altura &&
    bola.x + bola.largura >= jogadorDireita.x
  ) {
    bola.x = jogadorDireita.x - bola.largura; // Ajusta a posição da bola para evitar múltiplas colisões
    bola.dirx = -1;
    bola.mod += 0.2;
    reproduzirSom(somBatida);
  ...
  // Verifica se alguém marcou ponto (após colisões com as raquetes)
